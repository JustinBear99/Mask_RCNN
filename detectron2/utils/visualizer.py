...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        self.overlay_instances_asparagus(
            masks=masks,
            boxes=boxes,
            labels=labels,
            classes=classes,
    ...
    def overlay_instances_asparagus(
        self,
        *,
        boxes=None,
        labels=None,
        ...
        label_table = {1:'clump', 2:'stalk', 3:'spear', 4:'bar'}
        if areas is not None:
            sorted_idxs = np.argsort(-areas).tolist()
            # Re-order overlapped instances in descending order.
            boxes = boxes[sorted_idxs] if boxes is not None else None
            labels = [labels[k] for k in sorted_idxs] if labels is not None else None
            classes = [classes[k].tolist() for k in sorted_idxs] if classes is not None else None
            masks = [masks[idx] for idx in sorted_idxs] if masks is not None else None
            assigned_colors = [assigned_colors[idx] for idx in sorted_idxs]
            keypoints = keypoints[sorted_idxs] if keypoints is not None else None
        ...
        for i in range(num_instances):
            color = assigned_colors[i]
            if boxes is not None:
                if label_table[classes[i]] != 'stalk' and label_table[classes[i]] != 'spear' and label_table[classes[i]] != 'bar':
                    self.draw_box(boxes[i], edge_color=color)

            if masks is not None:
                for segment in masks[i].polygons:
                    if label_table[classes[i]] != 'clump':
                        self.draw_polygon(segment.reshape(-1, 2), color, alpha=alpha)
